...
/*
bool find(x1,y1,x2,y2)//路劲是否就是由x1,y1走到的x2,y2
{
	if(map_step[x1][y1]-map_step[x2][y2]==1) 
	return true;
	else return false;	
} 
*/
/*void find_step(*x,*y)
{
	...
	if(x>1) 
	if(find(x,y,x-1,y)) x-1;
	if(y<m) 
	if(find(x,y,x,y+1)) y+1;
	if(y>1) 
	if(find(x,y,x,y-1)) y-1;
}*/
int main()
...
//		cout<<p<<endl;
	...
	//	flag[line[line_now].x][line[line_now].y]=false;//移除更新队列 
	}
...
/*	if(map_step[start_x][start_y]==1<<30)
	printf("错误！\n");
	else
	while(map_step[start_x][start_y]!=0)
	{
		find_step(&start_x,&start_y);
